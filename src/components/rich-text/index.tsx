...
export function RichText({ children, content }: { children?: React.ReactNode; content?: any; components?: any }) {
  // Simple fallback for rich text
  return (
    <div className={richTextClasses}>
      {children || (typeof content === 'string' ? content : "Rich content placeholder")}
    </div>
  );
}
...
export function Code({
  ...
  if (isInline) {
    ...
    return (
      <pre className="rounded-lg border border-[--border] dark:border-[--dark-border]">{code || children}</pre>
    );
}

export function FaqRichtextComponent({ answer, _title }: { answer: string; _title: string }) {
  return (
...
export function RichTextCalloutComponent({
  ...
  size,
  content,
}: { _title: string, size?: "large" | "default", content: any }) {
  switch (size) {
    case "large":
      return (
        <article className={$richTextCallout({ size: "large" })} id={_title}>
...
export function RichTextImage(props: {
  ...
  return (
    <figure className="relative flex flex-col gap-2">
      <img alt={props.caption ?? "Image alt"} {...props} />
      {props.caption ? (
