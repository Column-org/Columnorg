...
type DarkLightImageProps = React.ImgHTMLAttributes<HTMLImageElement> & {
    alt?: string;
    dark?: ImageSource;
    light: ImageSource;
...
export function DarkLightImageAutoscale(props: DarkLightImageProps) {
  const [aspectRatioWidth, aspectRatioHeight] = props.light.aspectRatio
    ? props.light.aspectRatio.split("/").map(Number)
    : [props.light.width, props.light.height];
  ...
  return (
    <DarkLightImage
      ...
      style={{
        aspectRatio: props.light.aspectRatio ?? `${aspectRatioWidth}/${aspectRatioHeight}`,
      }}
      {...props}
    />
  );
