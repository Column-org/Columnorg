...
import { type PlanProps, type PricingTableProps } from ".";
...
function FeatureValue({
  ...
  const value = feature.values.items.find((v: any) => v.plan._id === activePlan);
  ...
  return (
    <td className="flex flex-1 items-center justify-end text-sm font-normal text-[--text-secondary] dark:text-[--dark-text-secondary]">
      {value.value?.__typename === "BooleanComponent" ? (
        value.value.boolean ? (
          <span className="flex items-center justify-center rounded-full bg-[rgba(var(--success-rgb),0.1)] p-1.5">
